...
Layout.ForceDirected = function(graph, options) {
	...
	this.max_iterations = options.iterations || 100;
	this.graph = graph;
	...
	this.init = function() {
		...
		// console.log("attraction multiplier: " + attraction_constant + " :: repulsion multiplier: " + repulsion_constant);
	};
	...
	this.generate = function() {
		if(layout_iterations < this.max_iterations && temperature > 1) {
		...
		//	var start = new Date().getTime();
			...
			for(var i=0; i < nodes_length; i++) {
				...
				var updated = true;
				node.position.x -=	(node.position.x-node.layout.tmp_pos_x)/20;
				node.position.y -=	(node.position.y-node.layout.tmp_pos_y)/20;
				node.position.z -=	(node.position.z-node.layout.tmp_pos_z)/20;
	...
	*/
	this.stop_calc = function() {
		previous_iteration = layout_iterations;
		layout_iterations = this.max_iterations;
	}
	
	this.start_calc = function() {
		layout_iterations = previous_iteration;
		this.finished = false;
	}
